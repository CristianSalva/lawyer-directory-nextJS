...
export default async function FirmsPage({ searchParams }: Props) {
  ...
  const areaParam = areaQuery ? `?area=${encodeURIComponent(area!)}&type=firm` : '?type=firm'
    ...
    <div>
      <div className="archive-header">
        <div className="container">
          <h1>{areaQuery ? `${area} Law Firms` : 'Find a Law Firm'}</h1>
        ...
        <div className="container">
          <div className="archive-results-header">
            {hasFilters ? (
              <p className="arc-count">
                ...
                {' — '}
                <Link href="/firms" className="llc-view-btn">Clear filters</Link>
              </p>
            ) : (
              <p className="arc-count">Browse law firms by state — <strong>{filtered.length}</strong> states</p>
            )}
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="attorneys-no-results">
              ...
              <br />
              <Link href="/firms" className="btn-notfound-primary">Browse all states</Link>
            </div>
          ) : (
